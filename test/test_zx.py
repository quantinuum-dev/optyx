...
def test_circuit2zx():
    ...
    assert qubit.Circuit(circuit).get_kraus() == Diagram.decode(
    ...
    t = qubit.Circuit(circuit).get_kraus()
    ...
    t = qubit.Circuit(circuit).to_pyzx().to_matrix()
    ...
    circuit = Ket(0)
    t = qubit.Circuit(circuit).to_pyzx().to_matrix() - _std_basis_v(0)
    assert np.isclose(np.linalg.norm(t), 0)
    circuit = Ket(0, 0)
    t = qubit.Circuit(circuit).to_pyzx().to_matrix() - _std_basis_v(0, 0)
    assert np.isclose(np.linalg.norm(t), 0)

    circuit = quantum.Id(3).CX(0, 2)
    assert (qubit.Circuit(circuit).get_kraus()
            == Diagram.decode(
                dom=Bit(3),
