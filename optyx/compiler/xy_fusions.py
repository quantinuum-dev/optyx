...
from optyx.compiler.x_fusions import (
        loss, min_trail_decomp,
compute_photons_with_x_fusions
)
...
def find_unbounded_trail_cover(g: nx.Graph) -> list[list[int]]:
...
def lc_xy(g: nx.Graph) -> nx.Graph:
    h, lcs = local_comp_reduction(g.copy(), loss)
    return h

def compliment_triangles_xy(g: nx.Graph) -> nx.Graph:
...
def photon_bounded_trail_cover_count(g: nx.Graph, photon_length: int) -> int:
    ...
    for i, trail in enumerate(trails):
        ...
        for node in trail:
            # If you see a node twice, it means theres an X fusion, so we need
            # just add the Y fusion photons once. It may be better to load
