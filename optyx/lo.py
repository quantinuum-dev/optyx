...
class TBS(Box):
    ...
    def to_zw(self, dtype=complex):
        ...
        sin = Z(lambda i: (backend.sin(self.theta * backend.pi)*phase) ** i, 1, 1)
        cos = Z(lambda i: (backend.cos(self.theta * backend.pi)*phase) ** i, 1, 1)
        minus_sin = Z(
            lambda i: (-backend.sin(self.theta * backend.pi)*phase) ** i, 1, 1
        )
